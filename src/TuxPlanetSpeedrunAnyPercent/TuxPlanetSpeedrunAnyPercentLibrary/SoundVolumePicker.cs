...
namespace TuxPlanetSpeedrunAnyPercentLibrary
{
	...
	public class SoundVolumePicker
	{
		...
		public SoundVolumePicker(int xPos, int yPos, int initialVolume, SoundAndMusicVolumePicker.Color color)
		{
			this._xPos = xPos;
			this._yPos = yPos;
		...
		public void Render(IDisplayOutput<GameImage, GameFont> displayOutput)
		{
			...
			switch (this._color)
			{
				case SoundAndMusicVolumePicker.Color.Black:
					image = this._currentVolume > 0 ? GameImage.SoundOn_Black : GameImage.SoundOff_Black;
					color = DTColor.Black();
					break;
				case SoundAndMusicVolumePicker.Color.White:
					image = this._currentVolume > 0 ? GameImage.SoundOn_White : GameImage.SoundOff_White;
					color = DTColor.White();
					break;
				default:
					throw new Exception();
